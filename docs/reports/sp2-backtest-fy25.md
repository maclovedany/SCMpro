...
## 라운드 2 (AI 조정 적용 후) — run_id `3f7498cf-262c-4d0d-95a8-c5c91a05eb52`
| 레벨 | 라운드 1 | 라운드 2 | Δ |
|---|---|---|---|
| 품목 WAPE | 31.8% | 32.3% | +0.5%p |
| 기종 WAPE | 31.4% | 31.9% | +0.6%p |

- 적용: Prophet off · Croston/SBA α 0.2 · LightGBM 600 trees / lr 0.04 · (MA3 window 5 는 효과 없어 3 으로 원복)
- **AI 1차 조정은 유의미한 개선을 주지 못함**(잡음 범위). 라운드 간 비교는 summary.vs_prev / regressed 로 자동 기록 (D-021).
- 사고: 제안에 포함된 `ol_bias` off 가 승인·적용되어 기종 WAPE 48.9% 로 급등 → 즉시 복구. 이후 챔피언 기법 off 를 거부하는 가드레일 추가 (D-021).
- 결론: 현재 데이터(3년)에서는 품목 대다수가 소량·간헐 수요라 단순 기법이 우세. 정교화 여지는 (a) EOL 마스터 반영(R-FC-07), (b) 장착률 기반 옵션 전개(R-BOM-04), (c) FY26 실적 누적 후 정직한 롤링 평가에 있음.
