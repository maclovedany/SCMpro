...
test("관리자 화면 렌더 + 품목 설정 승인 요청", async ({ page }) => {
  ...
  await page.goto("/admin/settings"); await expect(page.getByTestId("setting-flex_ranges")).toContainText("1번째 달 ±20%"); await expect(page.getByTestId("setting-notify_email_enabled")).toContainText("켬"); await expect(page.getByTestId("setting-agent_mode")).toContainText("끔"); await expect(page.getByText("ol_lead_months")).toHaveCount(0);
