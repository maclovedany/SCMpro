...
export function StackedBars({ categories, series, height = 220, colors, money = false, onClick }: Common & { categories: string[]; series: { name: string; data: number[] }[]; colors?: string[]; money?: boolean }) {
  const opt: EChartsOption = { ...base, tooltip: { ...base.tooltip, valueFormatter: (v) => (v == null ? "-" : money ? compact(Number(v)) : Number(v).toLocaleString("ko-KR")) }, xAxis: axisCat(categories), yAxis: axisVal(money ? compact : undefined),
...
export function Donut({ data, height = 200, centerLabel, colors, money = false, onClick }: Common & { data: { name: string; value: number }[]; centerLabel?: string; colors?: string[]; money?: boolean }) {
  ...
  const opt: EChartsOption = { ...base, tooltip: { trigger: "item", valueFormatter: (v) => fmt(Number(v)) }, legend: { orient: "vertical", right: 0, top: "middle", icon: "circle", itemWidth: 10, itemHeight: 10, textStyle: { color: TEXT.secondary, fontSize: 11 } },
