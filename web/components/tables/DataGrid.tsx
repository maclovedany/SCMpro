          ...
          <thead className="sticky top-0 z-10 bg-background shadow-[0_1px_0_0_var(--border)]">
            {table.getHeaderGroups().map(hg => (
              <tr key={hg.id}>{hg.headers.map(h => (
                <th key={h.id} className="cursor-pointer select-none whitespace-nowrap px-3 py-2 text-left font-medium text-muted-foreground" onClick={h.column.getToggleSortingHandler()}>
          ...
          <tbody>
            ...
            {items.map(vi => { const r = tRows[vi.index]; return (
              <tr key={rowKey(r.original)} data-index={vi.index} className={cn("border-t hover:bg-muted/40", onRowClick && "cursor-pointer")} onClick={() => onRowClick?.(r.original)}>
                {r.getVisibleCells().map(c => <td key={c.id} className="whitespace-nowrap px-3 py-1.5">{flexRender(c.column.columnDef.cell, c.getContext())}</td>)}
              </tr>); })}
