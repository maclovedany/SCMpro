...
const KIND_LABEL: Record<string, string> = { item_setting: "품목 설정", target_dos: "목표 DoS", allocation_mode: "배정방식", order_plan: "발주 계획", priority_alloc: "우선 배정", bulkdeal: "Bulkdeal 추가 발주", urgent_order: "긴급발주", forecast_tuning: "AI 예측 조정", agent_order: "AI 감시 발주 제안" };
...
export function describeApproval(a: { kind: string; target_pk: string; payload: Record<string, unknown> | null }, current?: Record<string, unknown> | null): string {
  ...
  if (a.kind === "urgent_order") { const p = a.payload ?? {}; return `긴급발주 요청: ${p.item_code} ${Number(p.qty ?? 0).toLocaleString("ko-KR")}개 · 필요일 ${p.need_date} · 요청 부서 ${p.dept} — 승인 시 추가수요로 반영 (R-OQ-44)`; }
