...
export function PlanDetail({ plan, lines, treeRows, months, filters, canEdit, total, page, pageSize }: Props) {
  ...
  const columns = useMemo<ColumnDef<LineRow, unknown>[]>(() => [
    ...
    { accessorKey: "on_hand", header: "현재고", cell: c => fmtInt(c.getValue() as number) },
    { accessorKey: "inbound_until_need", header: "입고예정", cell: c => fmtInt(c.getValue() as number) },
    { accessorKey: "forecast_need", header: "예측(필요월)", cell: c => fmtNum(c.getValue() as number, 1) },
    { accessorKey: "extras_need", header: "추가수요", cell: c => fmtInt(c.getValue() as number) },
    { accessorKey: "start_need", header: "기초(필요월)", cell: c => <span className={cn((c.getValue() as number) < 0 && "text-red-600")}>{fmtInt(c.getValue() as number)}</span> },
    { accessorKey: "target_stock", header: "목표재고", cell: c => fmtInt(c.getValue() as number) },
    { accessorKey: "required_qty", header: "필요량", cell: c => fmtInt(c.getValue() as number) },
    { id: "flex", header: "Flex 범위", accessorFn: r => r.flex_base != null ? `${fmtInt(r.flex_min)}~${fmtInt(r.flex_max)}${r.flex_hit ? " !" : ""}` : "-" },
    { accessorKey: "moq", header: "MOQ" },
    { accessorKey: "final_qty", header: "제안 발주", cell: c => <b>{fmtInt(c.getValue() as number)}</b> },
    { accessorKey: "override_qty", header: "오버라이드", cell: c => c.getValue() != null ? <Badge variant="secondary">{fmtInt(c.getValue() as number)}</Badge> : "" },
    { accessorKey: "dos_after", header: "발주 후 DoS", cell: c => <span className={cn(c.row.original.target_dos_days != null && (c.getValue() as number) < c.row.original.target_dos_days! && "text-amber-700")}>{fmtInt(c.getValue() as number)}</span> },
    { accessorKey: "amount", header: "금액", cell: c => `₩${fmtInt(c.getValue() as number)}` },
