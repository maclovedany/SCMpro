...
import { SalesOrderCharts } from "./SalesOrderCharts";
...
export default async function SalesOrdersPage({ searchParams }: { searchParams: Promise<{ item?: string; all?: string }> }) {
  ...
  const [orders, avail, customers] = await Promise.all([fetchMyOrders(sb, all === "1" || scm, p.user_id), item ? fetchAvailable(sb, item) : Promise.resolve(null), fetchCustomers(sb)]);
  return (
    <div className="space-y-4">
      ...
      <SalesOrderPanel orders={orders} avail={avail} item={item ?? ""} isScm={scm} me={p.user_id} now={now} customers={customers.map(c => ({ code: c.code, name: c.name }))} />
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2" data-testid="so-charts"><SalesOrderCharts orders={orders} now={now} /></div>
    </div>
  );
}
