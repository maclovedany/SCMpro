...
export function ExtraDemandForm() {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2 rounded-md border p-3 text-sm">
      ...
      {f.kind !== "meeting_approval" && <label>주문번호{f.kind === "confirmed_order" ? " *" : " (선택)"}<br /><Input name="order_no" className="mt-1 h-9 w-36" value={f.order_no} onChange={e => set("order_no", e.target.value)} /></label>}
      ...
      <Button size="sm" disabled={pending} onClick={() => start(async () => { const r = await addExtraDemand({ ...f, qty: Number(f.qty) }); if (r.ok) { const d = r.data as { bulkdeal_overlap?: { qty: number; customer: string | null; model: string | null }[] } | undefined; toast.success(f.kind === "bulkdeal" ? "등록 — 팀장 승인 요청됨" : "등록됨");
          if (d?.bulkdeal_overlap?.length) toast.warning(`이중 계상 주의: 같은 품목·필요월에 승인된 Bulkdeal ${d.bulkdeal_overlap.map(b => `${b.customer ?? "-"} ${b.qty}개`).join(", ")} 가 있습니다. 같은 거래면 한쪽을 취소하세요 (R-OQ-26)`, { duration: 12000 });
          setF({ ...f, item_code: "", qty: "", order_no: "" }); router.refresh(); } else toast.error(r.error); })}>등록</Button>
    </div>
  );
}
