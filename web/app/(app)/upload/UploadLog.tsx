  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody>{rows.map(r => { const errs = (r.errors ?? []) as { row: number; message: string }[]; return (
        <>
          <tr key={r.id} className="cursor-pointer border-t hover:bg-muted/40" onClick={() => setOpen(open === r.id ? null : r.id)}>
          ...
          {open === r.id && errs.length > 0 && <tr key={r.id + "e"}><td colSpan={6} className="bg-muted/30 p-2"><ul className="max-h-60 overflow-auto text-xs">{errs.map((e, i) => <li key={i}>행 {e.row}: {e.message}</li>)}</ul></td></tr>}
        </>); })}</tbody>
    </table>
  );
}
