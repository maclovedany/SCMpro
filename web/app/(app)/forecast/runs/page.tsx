...
export default async function RunsPage() {
  ...
  return (
    <div className="space-y-4">
      <div><h1 className="text-xl font-semibold">예측 런</h1><p className="text-sm text-muted-foreground">백테스트(평가 FY) 와 프로덕션 예측 이력. 요청한 런은 엔진(`engine forecast pending`)이 처리합니다.</p></div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>{runs.map(r => { const s = (r.summary ?? {}) as Record<string, number | null>; return (
          <tr key={r.id} className="border-t hover:bg-muted/40">
            ...
            <td><Badge variant="outline">{r.run_type}</Badge></td><td>{r.run_type === "backtest" ? `FY${String(r.eval_fy ?? "").slice(2)}` : `${r.horizon}개월`}</td>
