...
export function ItemsTable({ rows }: { rows: ItemMasterRow[] }) {
  ...
  const columns = useMemo<ColumnDef<ItemMasterRow, unknown>[]>(() => [
    ...
    { accessorKey: "avg_6m", header: "6M 평균", cell: c => <span className="tabular-nums">{fmtNum(c.getValue() as number)}</span> },
    { accessorKey: "total_12m", header: "12M 합", cell: c => <span className="tabular-nums">{fmtInt(c.getValue() as number)}</span> },
    { accessorKey: "on_hand", header: "현재고", cell: c => <span className="tabular-nums">{fmtInt(c.getValue() as number)}{c.row.original.stock_is_dummy && <Badge variant="secondary" className="ml-1 text-[10px]">더미</Badge>}</span> },
    { accessorKey: "inbound_qty", header: "입고예정", cell: c => <span className="tabular-nums">{fmtInt(c.getValue() as number)}</span> },
    { accessorKey: "dos_days", header: "DoS", cell: c => <span className="tabular-nums">{fmtInt(c.getValue() as number)}</span> },
    { accessorKey: "target_dos_days", header: "목표 DoS", cell: c => <span className="tabular-nums">{fmtInt(c.getValue() as number)}</span> },
